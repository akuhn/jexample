...
/**
 * Declares options for JExample test-classes. The options provided by this
 * annotations let you configure how values are passed from test method to test
 * method. For each executed test-method, the produced return value is cached.
 * When another method requests such a cached return value, the
 * <code>JExampleOptions</code> annotation of its containing class decides how
 * the return value is passed to the consumer.
 * 
...
public @interface JExampleOptions {
     ...
     * considered to be uncloneable. Immutable objects are not cloned, even when
     ...
     * than one consumer without caution, side-effects might appear. Such
     * side-effect are hard to track down (see Heisenbug), thus use this setting
