...
public class TestGraph {
	...
	private TestGraph() {
		...
		this.classInstances = new HashMap< Class<?>,Object>();
	}
	...
	public void addClass( TestClass testClass ) throws InitializationError {
		...
		this.validate( methodsUnderTest, testClass ); // validate the methods
														// of the testClass

		// if everything is fine, add the testClass to the list of classes under
		// test
		// and add all the testmethods and its dependencies to the list of
		// testmethods
	 ...
	 * added as childs to the class description. If there are dependencies from
	 * {@link TestMethod}'s which are not declared in a {@link TestClass} that
	 * is run in this turn, the {@link Description} of the declaring
	...
	private TestMethod addTestMethod( Method method ) throws IllegalArgumentException, SecurityException, InstantiationException, IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		TestMethod testMethod;
		if ( !this.testMethods.containsKey( method ) ) {
			testMethod = new TestMethod( method, this.getInstanceOfDeclaringClass( method ) );
	...
	private Object getInstanceOfDeclaringClass( Method method ) throws IllegalArgumentException, SecurityException, InstantiationException, IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		Object instance;
		if ( this.classInstances.containsKey( method.getDeclaringClass() ) ) {
			instance = this.classInstances.get( method.getDeclaringClass() );
		} else {
			instance = method.getDeclaringClass().getConstructor().newInstance();
			this.classInstances.put( method.getDeclaringClass(), instance );
		}
		return instance;
	}
